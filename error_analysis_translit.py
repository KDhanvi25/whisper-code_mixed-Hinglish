...
def main():
    if len(sys.argv) != 3:
        print("Usage: python3 error_analysis_translit.py ground_truth.json baseline_outputs.json")
        return
    ...
    print("📊 Word-level Error Breakdown (Original):")
    ...
    # Transliterated WER/CER
    refs_translit = [translit(r) for r in refs]
    hyps_translit = [translit(h) for h in hyps]

    translit_overall = process_words(refs_translit, hyps_translit)
    translit_cer_score = cer(refs_translit, hyps_translit)

    print("\n🌐 Transliterated WER/CER:")
    print(f"  🔁 WER (Latin): {translit_overall.wer * 100:.2f}%")
    print(f"  🔁 CER (Latin): {translit_cer_score * 100:.2f}%")
